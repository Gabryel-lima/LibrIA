...
def preprocess_frame(frame):
    ...
    resized = cv2.resize(rgb, (CFG_VGG16.IMG_SIZE, CFG_VGG16.IMG_SIZE)) # 224x224
...
def apply_gradcam(model, inp, frame, target_layer="block5_conv3"):
    """
...
def cam_imagenet(use_gradcam=True, target_layer="block5_conv3"):
    model = load_model(CFG_VGG16.MODEL_PATH)
    cap = open_camera()
    ...
    while True:
        ...
        label = CFG_VGG16.LABELS[class_idx]

        if use_gradcam:
            overlay, _ = apply_gradcam(model, inp, frame, target_layer=target_layer)
        else:
            overlay = frame.copy()
