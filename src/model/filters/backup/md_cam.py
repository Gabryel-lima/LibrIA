...
def inference_with_gradcam_and_hands(model_path, source=0, target_layer_name='conv5_3', inference_fps=10):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = ASLNetVGG().to(device) # feature_dim=758 v3
    ...
    tlay = getattr(model, target_layer_name, None) or model.conv5_3
    print(f"Grad-CAM em {tlay}")
    # mp_hands = mp.solutions.hands
    ...
    try:
        while True:
            ...
            # c) desenha os landmarks detectados
            if results.multi_hand_landmarks:
            ...
            # e) faz a predição e obtém o rótulo
            with torch.no_grad():
                pred = model(inp).softmax(1).argmax(1).item()
            label = CFG.LABELS[pred]
            cv2.putText(
                overlay, f"Pred: {label}", (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2
            )

            # f) exibe o resultado na janela
            cv2.imshow('Hands + Grad-CAM', overlay)

            # g) throttle para manter FPS constante
            elapsed = time.time() - loop_start
            if elapsed < frame_interval:
                time.sleep(frame_interval - elapsed)

            # h) condicional de saída ('q' para sair)
...
if __name__ == "__main__":
    inference_with_gradcam_and_hands(
        "src/model/filters/backup/v1_static_91.pt",
        source=2,
        target_layer_name='conv5_3',
        inference_fps=15  # ajuste conforme FPS usado no treino
    )
