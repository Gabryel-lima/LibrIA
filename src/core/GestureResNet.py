...
#     def residual_block(self, x: KerasTensor, filters: int = 64, kernel_size: int = 3, stride: int = 1) -> KerasTensor:
#             """
#             Cria um bloco residual.
...
#             )(x)
#             x = layers.BatchNormalization()(x)
#             x = layers.Dropout(0.3)(x)  # Dropout após a segunda convolução
...
#                 )(shortcut)
#                 shortcut = layers.BatchNormalization()(shortcut)
#                 shortcut = layers.Activation('relu')(shortcut)  # ReLU após ajuste do atalho

#             # Adiciona a conexão de atalho e aplica ReLU
#             x = layers.Add()([x, shortcut])
#             x = layers.Activation('relu')(x)

#             # Bloco de Squeeze and Excitation opcional
#             x = self.squeeze_excite_block(x)

#             return x

#     def squeeze_excite_block(self, x: KerasTensor, ratio: int = 16) -> KerasTensor:
#         """
#         Adiciona um bloco de Squeeze and Excitation para recalibrar os canais.
#         """
#         filters = x.shape[-1]
...
#         Retorna:
#         - Model: O modelo Keras da GestureResNet.
#         """
#         # Entrada do modelo
#         inputs = layers.Input(shape=input_shape)
...
#         # Camada de pooling global
#         x = layers.GlobalAveragePooling2D()(x)
#         x = layers.Dropout(0.3)(x)  # Dropout global antes da classificação
